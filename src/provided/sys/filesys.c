 ...
 */
 int parse_path(char* path, char** mpnameptr, char** flnameptr) {
    ...
    /* Root listing: "/" */
    if (path[0] == '/' && path[1] == '\0') {
        *mpnameptr = path;      // points to "/"
        path[0] = '\0';         // now empty string
        *flnameptr = NULL;
        return 0;
    }
    ...
    /* No slash at all -> invalid path for this kernel API */
    if (*c == '\0')
        return -EINVAL;

    /* Empty mountpoint like "/foo" is invalid */
    if (c == path)
        return -EINVAL;

    *c = '\0';
    *mpnameptr = path;
    *flnameptr = c + 1;   // may be empty string, and that is OK
...
//     while (*c != '/'){     //iterates until / is found or filename is reached
//         if (*c == '\0')
//             return -EINVAL;
//         c++;
//     }
...
//     *c = '\0';  //split strings at the / by adding the string end value
...
//     //this broke my code since it stopped mpnameptr from pointing at just mpname but instead pointed at the whole path again
//     //revisit if path parsing issues presist
//     //*c = '/';  //restore '/' in file path so that it can be used again 
