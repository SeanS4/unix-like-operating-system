...
#_thread_startup(void)
#Inputs: None (executes on a freshly created thread; s0–s7 contain a0–a7, s8 contains the entry PC).
#Outputs: None
#Description: Trampoline for new threads. Restores arguments by moving s0–s7 → a0–a7, loads the entry function pointer from s8 into t0, then jalr t0 to start the thread function. 
#When the thread function returns, control jumps to running_thread_exit.
#Side Effects: Consumes callee-saved registers used as staging (s0–s8); begins user thread execution; on return, terminates the thread.
_thread_startup:
