...
smode_trap_entry_from_umode:
        sd      t6, T6(sp)         #store old t6 on stack

        csrr    t6, sscratch
        sd      t6, SP(sp)
        csrw    sscratch, zero

        #save gp and tp from user mode into trap frame
        sd      gp, GP(sp)
        sd      tp, TP(sp) 
        ...
        # Restore _t6_ and _sp_ last    (also gp and tp for umode)
